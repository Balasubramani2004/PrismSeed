...
const SalarySlips: React.FC = () => {
  ...
  const handleDownload = (slip: SalarySlip, format: ExportFormat) => {
    ...
    console.log(`Downloading ${slip.month}/${slip.year} as ${format}`);
    alert(`Downloading ${new Date(slip.year, slip.month - 1).toLocaleString('default', { month: 'long' })} ${slip.year} salary slip as ${format.toUpperCase()}`);
  };
